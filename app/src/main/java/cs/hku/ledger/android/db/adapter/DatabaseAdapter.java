...
package cs.hku.ledger.android.db.adapter;
...
public abstract class DatabaseAdapter<Model extends BaseModel> {
     ...
     * @param model {@link BaseModel} from which to extract values
     * @return {@link ContentValues} with the data to be inserted into the db
     */
	...
	public Cursor fetchRecord(long rowId){
		return mDb.query(mTableName, null, CommonColumns._ID + "=" + rowId,
				null, null, null, null);
	}
    ...
    public long getID(@NonNull String uid){
        ...
        try{
            if (cursor.moveToFirst()) {
                result = cursor.getLong(cursor.getColumnIndexOrThrow(CommonColumns._ID));
            } else {
    ...
    public String getUID(long id){
        Cursor cursor = mDb.query(mTableName,
                new String[]{CommonColumns.COLUMN_UID},
        ...
        try {
            if (cursor.moveToFirst()) {
                uid = cursor.getString(cursor.getColumnIndexOrThrow(CommonColumns.COLUMN_UID));
            } else {
